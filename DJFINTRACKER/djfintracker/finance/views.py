...
class DashboardView(LoginRequiredMixin, View):
    def get(self, request, *args, **kwargs):
        ...
        total_income = Transaction.objects.filter(user=request.user, transactions_types='Income').aggregate(Sum('amount'))['amount__sum'] or 0

        total_expense = Transaction.objects.filter(user=request.user, transactions_types='Expense').aggregate(Sum('amount'))['amount__sum'] or 0
